...
export class CometChatUIKit {
  ...
  static async getLoggedInUser(): Promise<CometChat.User> {
    ...
    let user = await CometChat.getLoggedinUser().catch((e) =>
      Promise.reject(e),
    );
    if (user == null) {
      Promise.reject(
  ...
  static async login({
    ...
    if (CometChatUIKit.checkAuthSettings(Promise.reject)) null;
    if (uid) {
      let user = await CometChat.login(
        uid,
        CometChatUIKit.uiKitSettings?.authKey,
      ).catch((e) => Promise.reject(e));
      this.enableExtensions();
      return user;
    }
    if (authToken) {
      let user = await CometChat.login(authToken).catch((e) =>
        Promise.reject(e),
      );
      this.enableExtensions();
      return user;
  ...
  static sendCustomMessage(
    ...
    onSuccess?: (msg: CometChat.CustomMessage | CometChat.BaseMessage) => void,
    onError?: (msg: CometChat.CometChatException) => void,
  ): void {
    CometChatUIKitHelper.onMessageSent(message, messageStatus.inprogress);
    CometChat.sendCustomMessage(message)
      .then((customMessage) => {
        CometChatUIKitHelper.onMessageSent(
          customMessage,
  ...
  static sendMediaMessage(
    ...
    onSuccess?: (msg: CometChat.MediaMessage | CometChat.BaseMessage) => void,
    onError?: (msg: CometChat.CometChatException) => void,
  ): void {
    let hasAttachment;
    try {
    ...
    if (hasAttachment == undefined) {
      ...
      let attachmentObject: CometChat.Attachment = new CometChat.Attachment(
        file,
      );
      attachmentObject.setName(file['name']);
      attachmentObject.setExtension(
        (file['name'].lastIndexOf('.') + 1).toString(),
      );
  ...
  static sendTextMessage(
    ...
    onSuccess?: (msg: CometChat.TextMessage | CometChat.BaseMessage) => void,
    onError?: (msg: CometChat.CometChatException) => void,
  ): void {
    CometChatUIKitHelper.onMessageSent(message, messageStatus.inprogress);
