...
export const CometChatCallLogDetails = (
  props: CometChatCallLogDetailsConfigurationInterface,
) => {
  const {
    onBack,
    onError,
  ...
  useEffect(() => {
    CometChat.getLoggedinUser().then(
      (loggedUser: CometChat.User) => {
        ...
        let user =
          callData?.getReceiverType() == 'user'
            ? loggedInUser.current?.getUid() ===
              callData?.getInitiator()?.getUid()
              ? callData.getReceiver()
              : callData?.getInitiator()
            : undefined;
        let group =
          callData?.getReceiverType() == 'group'
            ? loggedInUser.current?.getUid() ===
              callData?.getInitiator()?.getUid()
              ? callData.getReceiver()
              : callData?.getInitiator()
        ...
        if (data) {
          ...
          let detailsTemplate = CallingDetailsUtils.getDefaultDetailsTemplates(
            callData,
            loggedInUser.current,
            theme,
          );
          detailsTemplate.push(
            CallingDetailsUtils.getSecondaryDetailsTemplate(
              callData,
              loggedInUser.current,
              theme,
            ),
  ...
  return (
    <View
      style={{ flex: 1, backgroundColor: _callLogDetailsStyle.backgroundColor }}
    >
              ...
              {showCloseButton && (
                <TouchableOpacity onPress={onBack}>
                  <Image
                    style={[
                      Style.imageStyle,
                      { tintColor: _callLogDetailsStyle.closeIconTint },
                    ]}
                    source={closeButtonIconImage || CloseIcon}
                  />
                </TouchableOpacity>
              )}
              <Text
              ...
              <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                ...
                <Text
                  style={[
                    {
                      marginVertical: 10,
                      color: _callLogDetailsStyle.titleColor,
                    },
                    _callLogDetailsStyle.titleFont,
                  ]}
                >
                  {user ? user['name'] : group ? group['name'] : undefined}
                </Text>
              </View>
              <SectionList
                sections={tempates}
