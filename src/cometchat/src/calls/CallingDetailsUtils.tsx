...
export class CallingDetailsUtils {
  ...
  ): Array<CometChatCallLogDetailsTemplate> {
    return [this.getPrimaryDetailsTemplate(callLog, loggedInUser, theme)];
  }

  static getPrimaryDetailsTemplate(
    callLog: any,
    loggedInUser: CometChat.User,
    theme: CometChatTheme,
  ): CometChatCallLogDetailsTemplate {
    let user =
      callLog?.getReceiverType() == 'user'
        ? loggedInUser?.getUid() === callLog?.getInitiator()?.getUid()
          ? callLog.getReceiver()
          : callLog?.getInitiator()
        : undefined;
    let group =
      callLog?.getReceiverType() == 'group'
        ? loggedInUser?.getUid() === callLog?.getInitiator()?.getUid()
          ? callLog.getReceiver()
          : callLog?.getInitiator()
        : undefined;
          ...
          >
            <View style={{ flexDirection: 'row' }}>
              <CometChatDate
                ...
                style={{
                  ...
                  textFont: theme.typography.subtitle1,
                }}
              />
              <Text
                style={[
            ...
            >
              {CallUtils.convertMinutesToHoursMinutesSeconds(
                callLog['totalDurationInMinutes'],
              )}
            </Text>
          </View>
        </View>
        ...
        CustomView: () => (
          <View
            style={{
              ...
              overflow: 'hidden',
              borderTopWidth: 0.5,
              borderColor: theme.palette.getAccent400(),
            }}
          >
            <CometChatListItem
              id={'history'}
            ...
            SubtitleView={() => (
              ...
              >
                ...
                <View style={{ flexDirection: 'row' }}>
                  <Text
                    style={[
                      { color: theme.palette.getAccent(), marginRight: 5 },
                      theme.typography.title2,
                    ]}
                  >
                    {callLog?.recordings?.length}
                  </Text>
                  <Image
                    source={NextArrowIcon}
                    style={{
