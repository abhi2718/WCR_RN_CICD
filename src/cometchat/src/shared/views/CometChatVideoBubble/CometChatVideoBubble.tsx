...
export const CometChatVideoBubble = (props: CometChatVideoBubbleInterface) => {
  const { videoUrl, thumbnailUrl, placeholderImage, style, playIcon, onPress } =
    props;
  ...
  return (
    <ImageBackground
      source={thumbnailUrl || placeholderImage}
      resizeMode={'contain'}
      style={{ backgroundColor:"#f9ebeb", ...border, borderRadius, height, width }}
    >
      <View
        onTouchEnd={playVideo}
        style={[
          Style.playIconPosition,
          { backgroundColor: playIconBackgroundColor, borderRadius },
        ]}
      >
        <Image
          source={playIcon || defaultPlayIcon}
          style={{ tintColor: playIconTint }}
        />
      </View>
    </ImageBackground>
  );
