...
} from '../shared/views/CometChatMessageBubble/MessageBubbleStyle';
...
const getColorByFirstLetter = (letter: any) => {
  ...
  const index = uppercaseLetter.charCodeAt(0) - 'A'.charCodeAt(0);
  const colorIndex = index % colors.length;

  return colors[colorIndex];
};

const UserNameText = ({ name }: any) => {
  const firstLetter = name.charAt(0);
  const color = getColorByFirstLetter(firstLetter);
...
>((props: CometChatMessageListProps, ref) => {
  ...
  _defaultRequestBuilder.setCategories(
  ...
  if (user)
    _updatedCustomRequestBuilder = _updatedCustomRequestBuilder.setUID(
      user['uid'],
    );
  if (group)
    _updatedCustomRequestBuilder = _updatedCustomRequestBuilder.setGUID(
      group['guid'],
    );
  if (parentMessageId)
    _updatedCustomRequestBuilder =
      _updatedCustomRequestBuilder.setParentMessageId(
  ...
  _updatedCustomRequestBuilder =
    _updatedCustomRequestBuilder.setCategories(categories);
  ...
  const markUnreadMessageAsRead = () => {
    if (
      messagesList[messagesList.length - 1].getReceiverType() ==
  ...
  const getPreviousMessages = async () => {
    ...
    messageRequest.current
      ...
      .then((msgs) => {
        ...
        reversed.length > 0 &&
          setMessagesList([...reversed.reverse(), ...messagesList]);
  ...
  const getUpdatedPreviousMessages = () => {
    ...
    if (user) messagesRequest = messagesRequest.setUID(user['uid']);
    if (group) messagesRequest = messagesRequest.setGUID(group['guid']);
    ...
    messagesRequest
      ...
      .then((updatedMessages) => {
        ...
        for (let i = 0; i < updatedMessages.length; i++) {
          let condition = (msg) =>
            msg.getId() == updatedMessages[i]?.actionOn.getId();
  ...
  function checkMessageInSameConversation(
    message: CometChat.BaseMessage,
  ): boolean {
    return (
  ...
  const newMessage = (newMessage, isReceived = true) => {
    ...
    if (baseMessage.getCategory() === MessageCategoryConstants.interactive) {
      newMessage =
        InteractiveMessageUtils.convertInteractiveMessage(baseMessage);
    }
    if (
      ...
      if (baseMessage.getParentMessageId()) {
        if (baseMessage.getParentMessageId() == parseInt(parentMessageId)) {
          ...
          let index = messagesList.findIndex(
            (msg) => msg.id === newMessage.parentMessageId,
          );
          let oldMsg: CometChat.BaseMessage = messagesList[index];
          oldMsg.setReplyCount(
            oldMsg.getReplyCount() ? oldMsg.getReplyCount() + 1 : 1,
          );
          let tmpList = [...messagesList];
      ...
      if (
        !scrollToBottomOnNewMessages &&
        Math.round(currentScrollPosition.current.y) <=
          currentScrollPosition.current.scrollViewHeight
      ) {
        if (
          (baseMessage.getSender()?.getUid() ||
            baseMessage?.['sender']?.['uid']) == loggedInUser.current?.['uid']
        ) {
          scrollToBottom();
          return;
        }
        ...
        if (baseMessage.getReceiverType() == ReceiverTypeConstants.user) {
          CometChatUIEventHandler.emitMessageEvent(
            MessageEvents.ccMessageDelivered,
            { message: newMessage },
          );
        }
  ...
  const messageEdited = (
    ...
    if (withMuid) condition = (msg) => msg['muid'] == editedMessage['muid'];
    else condition = (msg) => msg.getId() == editedMessage.getId();
    let msgIndex = messagesList.findIndex(condition);
    if (msgIndex > -1) {
      let tmpList = [...messagesList];
      if (
        editedMessage.getCategory() === MessageCategoryConstants.interactive
      ) {
        editedMessage =
          InteractiveMessageUtils.convertInteractiveMessage(editedMessage);
      }
  ...
  const updateMessageReceipt = (receipt) => {
    ...
    if (
      (tmpMsg as CometChat.BaseMessage).getReceiverType() ==
  ...
  useEffect(() => {
    ...
    CometChatUIEventHandler.addMessageListener(uiEventListener, {
      ...
      onInteractionGoalCompleted: (
        interactionReceipt: CometChat.InteractionReceipt,
      ) => {
        if (
          loggedInUser?.current?.['uid'] ===
          interactionReceipt?.getSender()?.getUid()
        ) {
    ...
    CometChatUIEventHandler.addGroupListener(uiEventListener, {
      ...
      ccGroupMemberAdded: ({ message, usersAdded, userAddedIn }) => {
        usersAdded.forEach((user) => {
          message[
            'message'
          ] = `${loggedInUser.current['name']} added ${user.name}`;
    ...
    if (
      ...
      return (
        <View>
          ...
          {image && name && user && (
            <View>
              <CometChatAvatar image={image} name={name} style={_avatarStyle} />
            </View>
          )}
        </View>
      );
  ...
  const getHeaderView = useCallback((item: CometChat.BaseMessage) => {
    ...
    if (
      ...
      item.getCategory() == MessageCategoryConstants.call
    )
      return null;
  ...
  const getFooterView = useCallback(
    (
      ...
      else if (isSender) messageState = 'WAIT';
          ...
          style={[
            {
              flexDirection: 'row',
              justifyContent:
                bubbleAlignment === 'right' ? 'flex-end' : 'flex-start',
            },
          ]}
        >
          <CometChatDate
  ...
  const getAlignment = useCallback(
    (item: CometChat.BaseMessage): MessageBubbleAlignmentType => {
  ...
  const privateMessage = (item: CometChat.BaseMessage) => {
    ...
    CometChat.getUser(item.getSender().getUid())
      .then((user) => {
        const userInfo = user;
        if (userInfo?.getName() && userInfo?.getUid()) {
          navigation.navigate(ROUTES.CommunityPrivateChat, {
            senderId: userInfo?.getUid(),
  ...
  const getStyle = useCallback(
    (item: CometChat.BaseMessage): BaseStyleInterface => {
      ...
      if (item.getCategory() == MessageCategoryConstants.interactive) {
        if (
          item.getType() === MessageTypeConstants.form ||
          item.getType() === MessageTypeConstants.card
        ) {
          _style.width = '100%';
        }
      }
  ...
  const openOptionsForMessage = useCallback(
    (item: CometChat.BaseMessage, template: CometChatMessageTemplate) => {
  ...
  const MessageView = useCallback(
    (params: {
      ...
      let hasTemplate = templatesMap.get(
  ...
  const RenderMessageItem = ({ item, index }) => {
    ...
    const previousMessageDate = messagesList[index - 1]
      ? new Date(getSentAtTimestamp(messagesList[index - 1]))
      : null;
      ...
      seperatorView = (
        <View style={{ marginBottom: 10 }}>
          <CometChatDate
            ...
            customDateString={
              dateSeperatorPattern
                ? dateSeperatorPattern(item['sentAt'])
                : undefined
            }
            dateAlignment="center"
          />
  ...
  const getErrorStateView = useCallback(() => {
    if (ErrorStateView)
      return ErrorStateView(
        new CometChat.CometChatException({ message: 'Something went wrong' }),
      );
    return (
      <View style={Style.msgContainerStyle}>
        <Text
  ...
  const getLoadingStateView = () => {
    ...
    return (
      <View style={Style.msgContainerStyle}>
        <ActivityIndicator
          size="large"
          color={messageListStyle?.loadingIconTint}
        />
      </View>
    );
  };
  ...
  function onContentSizeChange(contentWidth, contentHeight) {
    if (
      currentScrollPosition.current.y == 0 &&
      currentScrollPosition.current.scrollViewHeight
    ) {
      let diff = contentHeight - currentScrollPosition.current.scrollViewHeight;
      if (
        messagesLength.current > prevMessagesLength.current &&
        diff > 0 &&
        !(unreadCount > 0)
      ) {
        messageListRef.current.scrollTo({
          y:
            Platform.OS === 'ios'
              ? diff - currentScrollPosition.current.layoutHeight
              : diff,
          animated: false,
        });
      }
    ...
    if (currentScrollPosition.current.scrollViewHeight) {
      ...
      if (currentScrollPosition.current.scrollViewHeight !== contentHeight) {
        previousScrollPosition.current.scrollViewHeight =
          currentScrollPosition.current.scrollViewHeight;
  ...
  const { height, width, backgroundColor, border, borderRadius } =
    _messageListStyle;
  ...
  return (
    ...
    >
      {listState == 'loading' && messagesList.length == 0 ? (
        ...
        messagesList.length == 0 ? (
          ...
          <View style={{ height: '100%', width: '100%' }}>
            {HeaderView && (
              <View style={[Style.stickyHeaderFooterStyle, { top: 0 }]}>
                <HeaderView
            ...
            {loadingMessages && (
              <View style={{ position: 'absolute', alignSelf: 'center' }}>
                <ActivityIndicator
                  size="small"
                  color={_messageListStyle.loadingIconTint}
                />
              </View>
            )}
            ...
            {CustomListHeader && <CustomListHeader />}
            {FooterView && (
              <View style={[Style.stickyHeaderFooterStyle, { bottom: 0 }]}>
                <FooterView
                  group={group && group['guid']}
                  user={user && user['uid']}
                  id={{
      ...
      <CometChatBottomSheet
        ...
        isOpen={
          showMessageOptions.length > 0 || ExtensionsComponent || messageInfo
        }
      >
        {ExtensionsComponent ? (
          ExtensionsComponent
          ...
          <CometChatMessageInformation
            ...
            onBack={() => {
              infoObject.current = null;
              setMessageInfo(false);
              messageInformationConfiguration?.onBack &&
                messageInformationConfiguration.onBack();
            }}
          />
        ) : (
