...
export const CometChatConversations = (props: ConversationInterface) => {
  ...
  const ErrorView = () => {
    return (
      <View style={Style.listContainer}>
        <Text
          style={{ ..._style?.errorTextFont, color: _style?.errorTextColor }}
        >
  ...
  const userEventHandler = (...args) => {
    ...
    if (!blockedByMe) {
      let item = conversationListRef.current.getListItem(
        `${uid}_user_${loggedInUser.current.uid}`,
      );
      if (item) {
  ...
  const typingEventHandler = (...args) => {
    ...
    let conversation = conversationListRef.current.getListItem(
      `${args[0]['receiverType']}_${args[0]['receiverId']}`,
    );
    ...
    CometChat.CometChatHelper.getConversationFromMessage(newMessage)
      .then((conversation) => {
        if (newMessage.getCategory() === MessageCategoryConstants.interactive) {
          newMessage =
            InteractiveMessageUtils.convertInteractiveMessage(newMessage);
        }
        const oldConversation: CometChat.Conversation =
        ...
        if (newMessage['sender']['uid'] != loggedInUser.current?.['uid'])
          oldConversation.setUnreadMessageCount(
            oldConversation.getUnreadMessageCount() + 1,
          );
  ...
  const playNotificationSound = () => {
    ...
    CometChatSoundManager.play(
      customSoundForMessages ||
        CometChatSoundManager.SoundOutput.incomingMessageFromOther,
    );
  };
  ...
  const conversationClicked = (conversation) => {
    ...
    let index = selectedConversation.findIndex(
      (tmpConver) =>
        tmpConver.conversationWith.conversationId ==
        conversation.conversationWith.conversationId,
    );
    if (index < 0) {
  ...
  const LastMessageView = (params: {
    ...
    if (
      lastMessage &&
      !disableReadReceipt &&
      lastMessage['sender']['uid'] == loggedInUser.current?.uid
    ) {
      let status: MessageReceipt = 'ERROR';
      if (lastMessage?.hasOwnProperty('readAt')) status = 'READ';
      else if (lastMessage?.hasOwnProperty('deliveredAt')) status = 'DELIVERED';
      else if (lastMessage?.hasOwnProperty('sentAt')) status = 'SENT';
      readReceipt = disableReceipt ? null : (
        <CometChatReceipt
          receipt={status}
...
const TailView = (params: {
  ...
  React.useEffect(() => {
    ...
    CometChatUIEventHandler.addMessageListener(messageListenerId, {
      ...
      ccMessageRead: ({ message }) => {
        CometChat.CometChatHelper.getConversationFromMessage(message).then(
          (conversation) => {
            let conver = conversationListRef.current.getListItem(
              conversation.getConversationId(),
            );
            if (!conver) return;
    ...
    CometChatUIEventHandler.addGroupListener(groupListenerId, {
      ccGroupDeleted: ({ group }) => {
        conversationListRef.current?.removeItemFromList(
    ...
    return () => {
      ...
      CometChatUIEventHandler.removeConversationListener(
  ...
  const ConversationItemView = ({ item: conversation }) => {
    ...
    //custom view check
    if (ListItemView) return ListItemView(conversation);
    const { conversationWith, conversationType } = conversation;
    ...
    let image: ImageType,
      ...
      avatarIcon =
        conversationWith[conversationType == 'group' ? 'icon' : 'avatar'];
    ...
    if (selecting) {
      let index: number = selectedConversation.findIndex(
        (value) => value.conversationWith.conversationId == conversationId,
      );
      if (index >= 0) {
        image = rightTickIcon;
        backgroundColor = theme?.palette.getPrimary();
    ...
    return (
      <CometChatListItem
        ...
        SubtitleView={
          (SubtitleView && SubtitleView.bind(this, conversation)) ||
          (() => (
            <LastMessageView
        ...
        statusIndicatorColor={
          disableUsersPresence ? 'transparent' : backgroundColor
        }
        ...
        options={() =>
          options ? options(conversation) : getDefaultOptions(conversation)
        }
        unreadCount={conversation.unreadMessageCount}
      />
    );
