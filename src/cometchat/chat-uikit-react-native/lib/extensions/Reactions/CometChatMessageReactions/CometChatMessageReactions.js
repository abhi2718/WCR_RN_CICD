...
const CometChatMessageReactions = (props) => {
    const { messageObject, loggedInUser, updateReaction, style, onReactionClick,
    // theme,
     } = props;
    ...
    const getAddReactionButton = () => {
        ...
        <View>
          <Image source={ICONS.ADDREACTION} style={{
                height: 16,
                width: 16,
    ...
    const reactionView = (reactionObject, reactionData) => {
        ...
        let messageReactionsbackgroundColor = { backgroundColor: 'transparent' }, border = {
            borderWidth: 1,
        ...
        if (messageObject?.['type'] === 'text') {
            ...
            if (reactionData?.hasOwnProperty(loggedInUser?.['uid'])) {
                messageReactionsbackgroundColor = {
                    backgroundColor: messageObject?.['sender']?.['uid'] === loggedInUser?.['uid']
                        ? theme?.palette?.getBackgroundColor()
                        : theme?.palette?.getPrimary(),
                };
                border = {
        ...
        return count >= 1 ? (<TouchableOpacity onPress={() => {
    ...
    React.useEffect(() => {
        reactionRef.current = getExtentionData(messageObject, MetadataConstants.extensions.reactions);
        let isEmpty = false;
        if (reactionRef.current) {
            const messageReactions = Object.keys(reactionRef.current).map((data) => {
                if (Object.keys(reactionRef.current[data]).length) {
                    ...
                    return reactionView({ [data]: reactionData.current }, reactionData.current);
                }
                else {
                    isEmpty = false;
                }
    ...
    return reactionList.length ? (<View style={Styles.messageReactionListStyle}>
      <CometChatBottomSheet isOpen={isVisible} onClose={() => { }}>
        <CometChatEmojiKeyboard onClick={(item) => {
...
//# sourceMappingURL=CometChatMessageReactions.js.map