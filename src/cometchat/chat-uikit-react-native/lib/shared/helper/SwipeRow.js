...
import { Dimensions, Animated, PanResponder, StyleSheet, TouchableOpacity, View, } from 'react-native';
...
class SwipeRow extends Component {
    constructor(props) {
        ...
        if (this.props.forceCloseToRightThreshold &&
        ...
        if (this.props.forceCloseToLeftThreshold &&
            this.props.forceCloseToRightThreshold > 0) {
            this._translateX.addListener(({ value }) => {
                if (!this.isForceClosing &&
                    Dimensions.get('window').width - value <
        ...
        if (this.props.onLeftActionStatusChange &&
        ...
        if (this.props.onRightActionStatusChange &&
            this.props.rightActivationValue &&
            this.props.rightActivationValue < 0) {
            this._translateX.addListener(({ value }) => {
                const absValue = Math.abs(value);
                const isActivated = absValue > Math.abs(this.props.rightActivationValue);
    ...
    handleRightSwipe(projectedExtraPixels) {
        ...
        if (this.swipeInitialX < this.currentTranslateX) {
            if (this.currentTranslateX - projectedExtraPixels >
    ...
    renderVisibleContent() {
        ...
        return (<TouchableOpacity activeOpacity={1} onPress={this.combinedOnPress} accessible={false}>
    ...
    renderRowContent() {
        ...
        if (this.state.dimensionsSet) {
            return (<Animated.View manipulationModes={['translateX']} {...this._panResponder.panHandlers} style={{
                    zIndex: 2,
            ...
            return (<Animated.View manipulationModes={['translateX']} {...this._panResponder.panHandlers} onLayout={e => this.onContentLayout(e)} style={{
                    zIndex: 2,
                    transform: [{ translateX: this._translateX }],
                }}>
                    {this.renderVisibleContent()}
    ...
    render() {
        return (<View style={this.props.style ? this.props.style : styles.container}>
                <View style={[
                styles.hidden,
                {
