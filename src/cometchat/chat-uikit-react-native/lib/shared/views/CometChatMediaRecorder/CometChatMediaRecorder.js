...
export const CometChatMediaRecorder = (props) => {
    const { onClose, style, onPause, onPlay, onSend, onStop, onStart, mediaRecorderStyle, pauseIconUrl, playIconUrl, recordIconUrl, deleteIconUrl, stopIconUrl, submitIconUrl } = props;
    ...
    const { pauseIconTint, playIconTint, closeIconTint, stopIconTint, submitIconTint, audioBarTint, timerTextFont, timerTextstyle, timerTextColor, } = _style;
    useEffect(() => {
    ...
    const ImageButton = (props) => {
        ...
        return (<TouchableOpacity onPress={disabled ? () => { } : onClick} activeOpacity={disabled ? 1 : .5} style={buttonStyle}>
    ...
    const _render = ({ item, index }) => {
        return (<View style={[Style.soundBar, audioBarTint && { backgroundColor: audioBarTint }]}/>);
    };
    ...
    return (<View style={Style.container}>
            <View style={[Style.soundBarContainer]}>
                {Boolean(recordedFile) && <>
                    {!recordedPlaying ? <ImageButton image={playIconUrl || ICONS.PLAY} imageStyle={[Style.imageStyle, playIconTint && { tintColor: playIconTint }]} onClick={_onPlay} buttonStyle={[Style.buttonStyle, { width: "10%" }]}/> :
                <ImageButton image={pauseIconUrl || ICONS.PAUSE} imageStyle={[Style.imageStyle, pauseIconTint && { tintColor: pauseIconTint }]} onClick={_onPause} buttonStyle={[Style.buttonStyle, { width: "10%" }]}/>}
                </>}
            ...
            !Boolean(recordedFile) && { marginRight: 10 }
                    ...
                    <FlatList data={soundBars} keyExtractor={(item) => item} renderItem={_render} horizontal={true} style={{
            ...
            <View style={Style.buttonContainer}>
                <ImageButton image={deleteIconUrl || ICONS.DELETE} imageStyle={[Style.imageStyle, closeIconTint && { tintColor: closeIconTint }]} onClick={_onClose} buttonStyle={[Style.buttonStyle]}/>
                {Boolean(recordedFile) ?
            <ImageButton image={recordIconUrl || ICONS.MICROPHONE} imageStyle={[Style.imageStyle, stopIconTint && { tintColor: stopIconTint }]} onClick={_onStart} buttonStyle={[Style.buttonStyle]}/>
            : <ImageButton image={stopIconUrl || ICONS.STOP_PLAYER} imageStyle={[Style.imageStyle, stopIconTint && { tintColor: stopIconTint }]} onClick={_onStop} buttonStyle={[Style.buttonStyle]}/>}
                <ImageButton image={submitIconUrl || ICONS.SEND} imageStyle={[Style.imageStyle, submitIconTint && { tintColor: submitIconTint }]} onClick={_onSend} disabled={!Boolean(recordedFile)} buttonStyle={[Style.buttonStyle]}/>
            </View>
        </View>);
};
