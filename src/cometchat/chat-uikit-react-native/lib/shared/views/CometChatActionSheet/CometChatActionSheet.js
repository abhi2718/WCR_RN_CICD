...
const OptionListView = ({ key, title, iconUrl, iconTint, titleStyle, iconBackgroundColor, listItemIconBorderRadius, backgroundColor, cornerRadius, onPress, style, theme, }) => {
    return (<TouchableOpacity key={key} style={[
      ...
      <Image source={iconUrl} style={[
            Style.listItemImageStyle,
            {
                tintColor: iconTint || style?.listItemIconTint || 'rgba(20,20,20,0.69)',
                backgroundColor: iconBackgroundColor ||
                    style?.listItemIconBackground ||
                    'transparent',
                borderRadius: listItemIconBorderRadius || style?.listItemIconBorderRadius || 0,
            },
        ]}/>
      <Text style={[
            {
...
const OptionGridView = ({ title, iconUrl, iconTint, titleStyle, iconBackgroundColor, listItemIconBorderRadius, backgroundColor, cornerRadius, onPress, style, theme, }) => {
    return (<View style={[
        ...
        <Image source={iconUrl} style={[
            Style.listItemImageStyle,
            {
                ...
                backgroundColor: iconBackgroundColor ||
                    style?.listItemIconBackground ||
                    'transparent',
                borderRadius: listItemIconBorderRadius ||
...
export const CometChatActionSheet = (props) => {
    ...
    const _render = ({ item }) => {
        return (<OptionListView key={item.id} {...item} iconUrl={item.icon || item.iconUrl} theme={theme} style={style}/>);
    };
    const _renderGrid = ({ item }) => (<OptionGridView key={item.id} {...item} iconUrl={item.icon || item.iconUrl} theme={theme} style={style}/>);
    const getList = () => {
        if (listMode) {
            return (<View style={Style.listContainer}>
          <FlatList key={'list'} keyExtractor={(item) => item.id} data={actionList} numColumns={1} renderItem={_render} ItemSeparatorComponent={() => (<View style={{
                        height: 1,
                        backgroundColor: props.style?.actionSheetSeparatorTint ||
                            theme.palette.getAccent200(),
                    }}/>)}/>
        </View>);
        }
        else {
            return (<FlatList key={'grid'} keyExtractor={(item) => item.id} data={actionList} numColumns={2} renderItem={_renderGrid}/>);
        }
    };
    const getLayoutModeIcon = () => {
        if (props.hideLayoutMode)
            return null;
        let img = props.layoutModeIconURL;
        if (img == undefined) {
            img = listMode ? ICONS.GRID : ICONS.LIST;
        }
        return (<TouchableOpacity onPress={() => setListMode(!listMode)}>
        <Image style={[
